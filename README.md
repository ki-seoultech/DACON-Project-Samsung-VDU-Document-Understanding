...
# 2️⃣ Prepare dataset
python src/prep_data.py map-coco --dataset doclaynet --ann /path/to/train.json --images-dir /path/to/images --out-root ./unified_yolo

# 3️⃣ Train YOLO11m
yolo train \
model=yolo11m.pt \
data=/content/workspace/dataset_balanced/data.yaml \
imgsz=1280 \
batch=0.70 \
epochs=50 \
device=0 \
...
amp=True ema=True \
hyp=/content/hyp_doclayout_11m.yaml

# 4️⃣ Run inference
python src/infer_script.py
